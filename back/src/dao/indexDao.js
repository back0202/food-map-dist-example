...
exports.selectRestaurants = async function (connection, category) {
  const selectAllRestaurantsQuery = `SELECT title, address, videoUrl, category FROM Restaurants where status='A';`;
  const selectCaregorizedRestaurantsQuery = `SELECT title, address, videoUrl, category FROM Restaurants where status='A' and category=?;`;
  const Params = [category];
  const Query = category
    ? selectCaregorizedRestaurantsQuery
